...
var portfolioBound = false;

function renderPortfolio() {
  if (portfolioBound) return;
  portfolioBound = true;
...
function greeceNavInit() {
  ...
  var targets = Array.from(document.querySelectorAll('#section-greece .gr-section, #section-greece .gr-island'));
  var activeUpdateFrame = null;

  function setActiveGreeceNav(key) {
    navBtns.forEach(function(btn) {
      btn.classList.toggle('active', btn.dataset.gr === key);
    });
  }

  function updateActiveGreeceNav() {
    activeUpdateFrame = null;
    if (!targets.length) return;

    var anchorY = window.innerWidth <= 767 ? 132 : 180;
    ...
    for (var i = 0; i < targets.length; i += 1) {
      ...
      if (!activeTarget && rect.top > anchorY) {
        activeTarget = targets[i];
        break;
      }
    }

    if (!activeTarget) activeTarget = targets[targets.length - 1];
    setActiveGreeceNav(activeTarget.id.replace('gr-', ''));
  }

  function scheduleActiveGreeceNavUpdate() {
    if (activeUpdateFrame !== null) return;
    activeUpdateFrame = requestAnimationFrame(updateActiveGreeceNav);
  }
  ...
  navBtns.forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      setActiveGreeceNav(btn.dataset.gr);
      if (el) el.scrollIntoView({ behavior: 'smooth' });
    });
  });

  window.addEventListener('scroll', scheduleActiveGreeceNavUpdate, { passive: true });
  window.addEventListener('resize', scheduleActiveGreeceNavUpdate, { passive: true });
  scheduleActiveGreeceNavUpdate();
